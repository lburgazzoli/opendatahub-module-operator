...
IMG ?= ttl.sh/opendatahub-module-operator-$(shell git rev-parse --short HEAD 2>/dev/null || echo dev):1h
...
test-integration: prepare-integration test-integration-run ## Run integration tests (OpenShift; starts in-process manager).
...
VERSION_PKG    = github.com/lburgazzoli/opendatahub-module-operator/pkg/version
VERSION       ?= 0.0.0-dev
...
HELM_NAMESPACE ?= opendatahub-module-operator-system
HELM_RELEASE   ?= opendatahub-module-operator
HELM_EXTRA_ARGS ?=
...
KIND_CLUSTER         ?= opendatahub-module-operator
KIND_NODE_IMAGE      ?=
CERT_MANAGER_VERSION ?= v1.17.2

.PHONY: kind-create
kind-create: ## Create a Kind cluster with podman provider and install cert-manager.
...
undeploy-kustomize: ## Undeploy controller installed via kustomize.
	...
	if [ -n "$$out" ]; then echo "$$out" | $(KUBECTL) delete --ignore-not-found=$(ignore-not-found) -f -; fi
...
deploy-helm: ## Deploy controller via Helm chart.
	$(HELM) upgrade --install $(HELM_RELEASE) config/chart \
		...
		--set-string image.tag=$(word 2,$(subst :, ,$(IMG))) \
