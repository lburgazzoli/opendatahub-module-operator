# AGENTS.md — opendatahub-mlflow-operator

Standalone module operator reconciling the cluster-scoped singleton
`MLflowOperator` CR (API group `components.platform.opendatahub.io`).
...
- Reconciles only the singleton instance `default-mlflowoperator`
...
| `controller.leader-election.id` | `odh-mlflow-lock` |
...
| `make get-manifests` | Download embedded MLflow manifests |
