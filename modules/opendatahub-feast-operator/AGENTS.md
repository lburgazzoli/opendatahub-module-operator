# AGENTS.md — opendatahub-feast-operator

Standalone module operator reconciling the cluster-scoped singleton
`FeastOperator` CR (API group `components.platform.opendatahub.io`).
...
- Reconciles only the singleton instance `default-feastoperator`
...
- Injects `spec.oidc.issuerURL` into `OIDC_ISSUER_URL` before kustomize renders
  manifests
...
| `controller.leader-election.id` | `odh-feast-lock` |
...
and `.Values.config` are written into the operator ConfigMap. The chart also
sets `ODH_MODULE_OPERATOR_APPLICATIONS_NAMESPACE` to the release namespace.
...
| `make get-manifests` | Download embedded Feast manifests |
