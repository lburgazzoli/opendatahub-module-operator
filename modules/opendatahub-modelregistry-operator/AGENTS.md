# AGENTS.md — opendatahub-modelregistry-operator

Standalone module operator reconciling the cluster-scoped singleton
`ModelRegistry` CR (API group `components.platform.opendatahub.io`).
...
- Ensures the registries namespace exists before deploy
...
- Requires `spec.gateway.domain` to be set; reconcile fails if it is empty
...
**E2E tests** — requires a cluster, a published operator image, and
`GATEWAY_DOMAIN` set to the cluster ingress domain:
```sh
...
| `controller.leader-election.id` | `odh-modelregistry-lock` |
...
| `make get-manifests` | Download embedded Model Registry manifests |
