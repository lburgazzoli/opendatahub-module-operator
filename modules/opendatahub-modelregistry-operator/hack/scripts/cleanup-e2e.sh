...
NAMESPACE="${1:-opendatahub-modelregistry-operator-system}"
HELM_RELEASE="${2:-opendatahub-modelregistry-operator}"
CR_RESOURCE="modelregistries.components.platform.opendatahub.io"
...
kubectl delete clusterroles -l platform.opendatahub.io/part-of=model-registry-operator --ignore-not-found 2>/dev/null || true
kubectl delete clusterrolebindings -l platform.opendatahub.io/part-of=model-registry-operator --ignore-not-found 2>/dev/null || true
