...
func TestUpsert_SortedOrder(t *testing.T) {
	...
	releases.Upsert(status, common.ComponentRelease{Name: "workbenches", Version: "1.0.0"})
	releases.Upsert(status, common.ComponentRelease{Name: "platform", Version: "3.5.0"})

	g.Expect(status.Releases).To(HaveLen(2))
	g.Expect(status.Releases[0].Name).To(Equal("platform"))
	g.Expect(status.Releases[1].Name).To(Equal("workbenches"))
}
