...
func NewScheme() (*runtime.Scheme, error) {
	...
	if err := admissionv1.AddToScheme(s); err != nil {
		return nil, fmt.Errorf("adding admission scheme: %w", err)
	}
	if err := apiextensionsv1.AddToScheme(s); err != nil {
		return nil, fmt.Errorf("adding apiextensions scheme: %w", err)
	}
	if err := componentsv1alpha1.AddToScheme(s); err != nil {
		return nil, fmt.Errorf("adding workbenches scheme: %w", err)
	}
	if err := imagev1.Install(s); err != nil {
		return nil, fmt.Errorf("adding image scheme: %w", err)
	}
