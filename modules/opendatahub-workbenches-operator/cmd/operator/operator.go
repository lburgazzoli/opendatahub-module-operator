...
import (
	...
	libcache "github.com/lburgazzoli/opendatahub-module-operator/modules/opendatahub-workbenches-operator/pkg/cache"
...
func run(cmd *cobra.Command, _ []string) error {
		...
		Client: client.Options{
			Cache: &client.CacheOptions{
				...
				DisableFor: []client.Object{
					&corev1.ConfigMap{},
					&corev1.Secret{},
					// MLflowOperator: singleton check works even when CRD is absent.
					resources.GvkToUnstructured(gvkpkg.MLflowOperator),
					// OpenShift cluster Ingress: read by cluster.GetDomain, no informer needed.
	...
	if err := workbenches.NewReconciler(cmd.Context(), mgr, cfg, rel); err != nil {
		return fmt.Errorf("creating workbenches reconciler: %w", err)
	}
