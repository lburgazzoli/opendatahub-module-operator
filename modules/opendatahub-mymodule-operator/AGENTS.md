# AGENTS.md — opendatahub-mymodule-operator

Standalone module operator reconciling a cluster-scoped singleton
`MyModule` CR (API group `components.opendatahub.io`).
...
IMG="localhost/opendatahub-mymodule-operator:e2e-$(uuidgen | tr '[:upper:]' '[:lower:]')"
