# AGENTS.md — opendatahub-trustyai-operator

Standalone module operator reconciling the cluster-scoped singleton
`TrustyAI` CR (API group `components.platform.opendatahub.io`).
...
- Reconciles only the singleton instance `default-trustyai`
...
- Switches to the `mcp-guardrails` overlay when `spec.mcpGuardrailsMode=true`
- Applies 12 image substitutions to `params.env` once at startup from
  environment variables
- Renders the `openshift-config-grants` template from embedded manifests
- Creates a `trustyai-dsc-config` ConfigMap from the CR's eval permission
  fields
...
| `controller.leader-election.id` | `odh-trustyai-lock` |
...
| `make get-manifests` | Download embedded TrustyAI manifests |
