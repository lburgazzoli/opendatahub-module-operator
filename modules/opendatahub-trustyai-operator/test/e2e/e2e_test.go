...
func runTestMain(m *testing.M) int {
	...
	preconditions, err := support.EnsureTrustyAIPreconditionsIfMissing(context.Background(), cli)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Failed to ensure TrustyAI preconditions: %v\n", err)
		return 1
	}
	manageKserveCR = preconditions.ManageKserveCR
