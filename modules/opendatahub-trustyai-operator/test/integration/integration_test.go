...
import (
	...
	modulemanager "github.com/lburgazzoli/opendatahub-module-operator/modules/opendatahub-trustyai-operator/pkg/manager"
...
func runTestMain(m *testing.M) int {
	...
	if err := cli.Get(ctx, client.ObjectKeyFromObject(moduleCRD), moduleCRD); err != nil {
		fmt.Fprintf(
			...
			componentsv1alpha1.TrustyAICRDName,
			err,
		)
		return 1
	}

	if err := support.EnsureTrustyAIPreconditions(ctx, cli); err != nil {
		fmt.Fprintf(os.Stderr, "Failed to install TrustyAI preconditions: %v\n", err)
		return 1
	}

	_ = cli.DeleteAllOf(ctx, &componentsv1alpha1.TrustyAI{})
