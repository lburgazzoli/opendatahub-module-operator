# AGENTS.md — opendatahub-datasciencepipelines-operator

Standalone module operator reconciling the cluster-scoped singleton
`DataSciencePipelines` CR (API group
...
- Reconciles only the singleton instance `default-datasciencepipelines`
- Uses the ODH overlay for `platformType=OpenDataHub`
- Uses the RHOAI overlay for `platformType=SelfManagedRhoai` and
  `platformType=ManagedRhoai`
- Honors `spec.argoWorkflowsControllers.managementState`
  (`Managed` by default, `Removed` to leave bundled Argo controllers unmanaged)
- Blocks reconciliation when Argo ownership preconditions are not met
- Reports `ArgoWorkflowAvailable`, `Ready`, `ProvisioningSucceeded`, and
  `status.releases`
...
| `platformType` | `OpenDataHub` | Platform identifier used to select ODH vs RHOAI overlays |
...
| `controller.leader-election.id` | `odh-datasciencepipelines-lock` |
...
When deployed through Helm, `.Values.platform.type`,
`.Values.platform.version`, and `.Values.config` are written into the operator
ConfigMap. The chart also sets `ODH_MODULE_OPERATOR_APPLICATIONS_NAMESPACE` to
...
| `make get-manifests` | Download embedded DSP manifests |
