...
`SparkOperator` CR (API group `components.platform.opendatahub.io`).
...
- Reconciles only the singleton instance `default-sparkoperator`
...
- Applies controller and webhook image substitutions to `params.env` once at
...
- Leaves upgrade migrations as a dedicated hook in `sparkoperator_upgrade.go`
...
| `controller.leader-election.id` | `odh-spark-lock` |
...
| `make get-manifests` | Download embedded Spark manifests |
