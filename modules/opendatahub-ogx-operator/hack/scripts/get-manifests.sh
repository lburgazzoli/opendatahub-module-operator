...
if [[ "${USE_LOCAL:-}" == "true" ]] && [[ -d "${PROJECT_ROOT}/../ogx-k8s-operator" ]]; then
    echo "Copying manifests from adjacent ogx-k8s-operator checkout"
    rm -rf "${DST_MANIFESTS_DIR}"
    mkdir -p "${DST_MANIFESTS_DIR}"
    cp -a "${PROJECT_ROOT}/../ogx-k8s-operator/${SOURCE_PATH}/." "${DST_MANIFESTS_DIR}/"
