# AGENTS.md — opendatahub-ogx-operator

Standalone module operator reconciling the cluster-scoped singleton
`OGX` CR (API group `components.platform.opendatahub.io`).
...
- Reconciles only the singleton instance `default-ogx`
...
- Keeps the module-specific precondition check as a no-op in the split operator
...
| `controller.leader-election.id` | `odh-ogx-lock` |
...
| `make get-manifests` | Download embedded OGX manifests |
