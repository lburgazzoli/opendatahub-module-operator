...
func New(
	...
	mgrOpts := ctrl.Options{
		...
		Cache: cache.Options{
			...
			DefaultNamespaces: map[string]cache.Config{
				cfg.ApplicationsNamespace: {
					LabelSelector: k8slabels.SelectorFromSet(k8slabels.Set{
						odhLabels.PlatformPartOf: odhLabels.NormalizePartOfValue(componentsv1alpha1.OGXKind),
					}),
				},
				cache.AllNamespaces: {
					LabelSelector: k8slabels.SelectorFromSet(k8slabels.Set{
			...
			ByObject: map[client.Object]cache.ByObject{
				&componentsv1alpha1.OGX{}:                   {Label: k8slabels.Everything()},
	...
	if err := ogx.NewReconciler(ctx, mgr, cfg, cfg.Release()); err != nil {
		return nil, fmt.Errorf("creating ogx reconciler: %w", err)
	}
