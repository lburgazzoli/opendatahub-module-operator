...
import (
	...
	"github.com/lburgazzoli/opendatahub-module-operator/modules/opendatahub-ogx-operator/pkg/resources/gvk"
)
	...
	for i < len(lines) {
		...
		if trimmed == yamlFieldData {
			...
			result = append(result,
				...
				indent+"  {{- end }}",
				indent+`  platform-name: {{ default "OpenDataHub" (index .Values.config "platform-name") | quote }}`,
				indent+`  platform-version: {{ default "unknown" (index .Values.config "platform-version") | quote }}`,
				indent+"  {{- range $key, $val := .Values.config }}",
				indent+`  {{- if and (ne $key "platform-name") (ne $key "platform-version") }}`,
